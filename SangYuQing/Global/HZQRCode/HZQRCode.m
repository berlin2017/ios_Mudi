...
{
    CIImage *image = [HZQRCode createQRCodeImage:qrString];
    return [HZQRCode resizeQRCodeImage:image withSize:size];
}

+ (UIImage *)qrCodeImageWithString:(NSString *)qrString size:(CGFloat)size icon:(UIImage *)icon iconSize:(CGSize)iconSize
{
    UIImage *qrImage = [self qrCodeImageWithString:qrString size:size];
    return [self addIconToQRCodeImage:qrImage withIcon:icon withIconSize:iconSize];
}
